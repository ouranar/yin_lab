import rawSiteData from "../../content/site-content.json";
...
const normalizeBulletLines = (value: string) =>
  value
    .split(/\r?\n/)
    .map((line) => line.replace(/^[\s鈫掆€⒙穃-]+/, "").trim())
    .filter(Boolean);
...
const normalizeSiteData = (input: SiteData): SiteData => {
  ...
  data.recruitment = {
    ...
    intro: [
      "我们欢迎来自教育技术、计算机、数据科学、设计、人机交互等不同背景的同学和研究者加入，一起围绕真实教育与学习场景中的关键问题开展研究。",
      cards[0]?.content || "在这里，研究不会停留在概念层面，而是要真正走进课程、平台与学习过程，提出问题、构建方法、开发工具，并在现场持续验证它们是否有效。",
      notes[0] || "如果你期待把想法落到系统、把观察转成证据、把研究做成可以产生影响的成果，这里会是一个适合长期投入与成长的地方。",
    ].filter(Boolean),
    audience: {
      title: "我们在找谁",
      intro: "我们尤其欢迎以下类型的伙伴：",
...
export const getDefaultSiteData = (): SiteData =>
  normalizeSiteData(clone(rawSiteData as unknown as SiteData));

export const readSiteData = () => getDefaultSiteData();

export const getVisibleNews = (data: SiteData) =>
  sortByDateDesc(data.news.items.filter((item) => item.visible !== false));
